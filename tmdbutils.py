...
class TMDBUtils():
    ...
    def transform_result(self, result):
        ...
        if result['poster_path']:
            image_url = \
                self.config['images']['base_url'] + self.config['images']['poster_sizes'][0] + result['poster_path']
    ...
    def search(self, query, suggestions=False):
        ...
        return map(self.transform_result, response['results'][:5])
    ...
    def get_season(self, tmdb_id, season_number):
        return self.transform_result(tmdb.TV_Seasons(tmdb_id, season_number).info())
